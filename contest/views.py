...
class ContestAdminAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            groups = []
            # 首先判断作业的类型： 0 即为是小组赛(GROUP_CONTEST)，1 即为是无密码的公开赛(PUBLIC_CONTEST)，
            # 2 即为是有密码的公开赛(PASSWORD_PUBLIC_CONTEST)
            # 此时为有密码的公开赛，并且此时只能超级管理员才有权限此创建作业
            ...
            if data["contest_type"] in [PASSWORD_PROTECTED_CONTEST, PASSWORD_PROTECTED_GROUP_CONTEST]:
                if not data["password"]:
                    return error_response(u"此作业为有密码的作业，密码不可为空")
            # 没有密码的公开赛 没有密码的小组赛
            ...
            if data["start_time"] >= data["end_time"]:
                return error_response(u"作业的开始时间必须早于作业结束的时间")
            try:
                ...
                return error_response(u"作业名已经存在")
    ...
    def put(self, request):
        ...
        if serializer.is_valid():
            ...
            if data["contest_type"] == PASSWORD_PROTECTED_CONTEST:
                if not data["password"]:
                    return error_response(u"此作业为有密码的公开赛，密码不可为空")
    ...
    def put(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return error_response(u"该作业题目不存在！")
        ...
        if contest_problem_id:
            try:
                ...
                if request.user.admin_type != SUPER_ADMIN and contest_problem.created_by != request.user:
                    return error_response(u"作业题目不存在")
                return success_response(ContestProblemSerializer(contest_problem).data)
            except ContestProblem.DoesNotExist:
                return error_response(u"作业题目不存在")
    ...
    def post(self, request):
        ...
        try:
            ...
            if problem.contest.status != CONTEST_ENDED:
                return error_response(u"作业还没有结束，不能公开题目")
...
def contest_problem_page(request, contest_id, contest_problem_id):
    ...
    try:
        ...
        return error_page(request, u"作业题目不存在")
    ...
    if contest.status == CONTEST_ENDED:
        ...
        show_warning = True
        warning = u"作业没有开始，您是管理员，可以提交和测试题目，但是目前的提交不会计入排名。"
...
@login_required
def answerquestions_page(request, contest_id, contest_problem_id):
    """
    我作业单个题目的所有提交列表
    """
    try:
        ...
        return error_page(request, u"作业不存在")
    try:
        contest_problem = ContestProblem.objects.get(id=contest_problem_id, visible=True)
    except ContestProblem.DoesNotExist:
        return error_page(request, u"作业问题不存在")
    ...
    return render(request, "oj/problem/answerquestions.html",
...
def contest_problem_my_submissions_list_page(request, contest_id, contest_problem_id):
    ...
    submissions = Submission.objects.filter(user_id=request.user.id, problem_id=contest_problem.id, contest_id=contest_id). \
        order_by("-create_time"). \
        values("id", "result", "create_time", "accepted_answer_time", "language")
    return render(request, "oj/submission/contest_problem_my_submissions_list.html",
                  {"submissions": submissions, "problem": contest_problem})

@check_user_contest_permission
def contest_problem_submissions_list_page(request, contest_id, page=1):
    """
    单个作业中的所有提交（包含自己和别人，自己可查提交结果，其他人不可查）
    """
    try:
        contest = Contest.objects.get(id=contest_id)
    except Contest.DoesNotExist:
