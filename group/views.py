    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return error_response(u"班级名已经存在")
        ...
        if serializer.is_valid():
            ...
            if group.join_group_setting == 0:
                if join_group(request.user, group):
                    return success_response(u"你已经成功的加入该班级")
                else:
                    return error_response(u"你已经是班级成员了")
                ...
                return error_response(u"该班级不允许任何人加入")
        else:
            return serializer_invalid_response(serializer)
        ...
        if serializer.is_valid():
            ...
            if data["status"]:
                if join_group(join_request.user, join_request.group):
                    ...
                    return error_response(u"加入失败，已经在本班级内")
            else:
...
def group_page(request, group_id):
    try:
        ...
        return error_page(request, u"班级不存在")
    joined = True
    try:
